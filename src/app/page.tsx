...
const MAX_SELECTABLE_COL = SELECTABLE_COL_INDICES[SELECTABLE_COL_INDICES.length - 1]; // This is the Images column
...
export default function IssueTrackerPage() {
  ...
  const [internalClipboard, setInternalClipboard] = useState<InternalClipboardData | null>(null);

  const [draggedRowId, setDraggedRowId] = useState<string | null>(null);
  const [rowDragOverId, setRowDragOverId] = useState<string | null>(null);

  // State for Image Editor Modal
  const [editingImageInfo, setEditingImageInfo] = useState<{ issueId: string; image: IssueImage } | null>(null);
  ...
  useEffect(() => {
    ...
    if (storedIssues) {
      try {
        const parsedIssues: Omit<Issue, 'images'> & { images: Omit<IssueImage, 'file'>[] }[] = JSON.parse(storedIssues);
        setIssues(parsedIssues.map(issue => ({
          ...issue,
          images: issue.images.map(img => ({
            ...img,
            file: new File([], img.name || "image.png", { type: img.url.match(/data:(image\/\w+);/)?.[1] || "image/png" }),
          }))
        })));
      } catch (error) {
  ...
  const copySelectedCellsToInternalClipboard = useCallback(() => {
    ...
    let minColActual = Infinity, maxColActual = -Infinity;
    ...
    const selectedColsInOrder = SELECTABLE_COL_INDICES.filter(c => c >= minColActual && c <= maxColActual);
      ...
      for (const colIndex of selectedColsInOrder) {
        if (selectedCells.has(`${r}-${colIndex}`)) {
          if (colIndex === SELECTABLE_COL_INDICES[0]) { 
            ...
            rowData.push(currentIssue.images.map(img => ({ url: img.url, name: img.name })));
          } else {
            rowData.push(null);
          }
        } else {
    ...
    toast({ title: "Selection Copied", description: `${clipboardData.flat().filter(c => c !== null).length} cell(s) copied to clipboard.` });
  ...
  const pasteFromInternalClipboard = useCallback(() => {
    ...
    const activeCellSelectableIndex = SELECTABLE_COL_INDICES.indexOf(activeCell.col);

    if (activeCellSelectableIndex === -1) {
        toast({title: "Paste Error", description: "Cannot paste starting from this cell.", variant: "destructive"});
        return;
    }
    ...
    internalClipboard.forEach((clipboardRow, deltaRow) => {
      ...
      while (targetRowIndex >= newIssues.length) {
        newIssues.push({ id: crypto.randomUUID(), description: "", images: [] });
      }
      ...
      clipboardRow.forEach((cellData, deltaColInClipboard) => {
        ...
        const targetColSelectableArrayIndex = activeCellSelectableIndex + deltaColInClipboard;
        if (targetColSelectableArrayIndex >= SELECTABLE_COL_INDICES.length) return; 

        const actualTargetCol = SELECTABLE_COL_INDICES[targetColSelectableArrayIndex];

        if (actualTargetCol === SELECTABLE_COL_INDICES[0]) { // Target is Description column
          if (typeof cellData === 'string') {
            targetIssue.description = cellData;
          } else {
            // Do not paste non-string data into description
          }
        } else if (actualTargetCol === SELECTABLE_COL_INDICES[1]) { // Target is Images column
          if (Array.isArray(cellData) && (cellData.length === 0 || (cellData.length > 0 && typeof cellData[0] === 'object' && cellData[0] !== null && 'url' in cellData[0] && 'name' in cellData[0]))) {
            const imagesToPaste = cellData as Pick<IssueImage, 'url' | 'name'>[];
            if (imagesToPaste.length > 0) {
              const newImagesForCell: IssueImage[] = imagesToPaste.map(imgData => ({
                id: crypto.randomUUID(),
                url: imgData.url,
                name: imgData.name,
                file: new File([], imgData.name, { type: imgData.url.match(/data:(image\/\w+);/)?.[1] || "image/png" }),
              }));
              targetIssue.images = [...(targetIssue.images || []), ...newImagesForCell];
            }
          } else {
    ...
    setIssues(newIssues);
    toast({ title: "Pasted", description: "Content pasted from internal clipboard." });
  }, [internalClipboard, activeCell, issues, toast]);

  useEffect(() => {
    const handleDocumentKeyDown = (event: KeyboardEvent) => {
      if (!tableContainerRef.current || !document.activeElement || !tableContainerRef.current.contains(document.activeElement)) {
        // If focus is not within the table, or table isn't focused, don't interfere with global shortcuts
      }

      if (event.ctrlKey || event.metaKey) {
        if (event.key.toLowerCase() === 'c') {
          if (selectedCells.size > 0 && tableContainerRef.current?.contains(document.activeElement)) {
            copySelectedCellsToInternalClipboard();
            event.preventDefault();
          }
        } else if (event.key.toLowerCase() === 'v') {
          if (internalClipboard && activeCell && tableContainerRef.current?.contains(document.activeElement)) { 
  ...
  }, [selectedCells, copySelectedCellsToInternalClipboard, internalClipboard, activeCell, pasteFromInternalClipboard]);


  const updateSelectedCellsRange = useCallback((start: { row: number; col: number }, end: { row: number; col: number }) => {
  ...
  const handleCellMouseDown = useCallback((row: number, col: number, event: React.MouseEvent) => {
    ...
    if (col === 0) { // Click on S/N column for row selection
      setIsDragging(false);
      const newSelectedCells = new Set<string>();
    ...
    if (event.shiftKey && selectionAnchor) {
      const effectiveAnchor = SELECTABLE_COL_INDICES.includes(selectionAnchor.col) ? selectionAnchor : {...selectionAnchor, col: MIN_SELECTABLE_COL};
    ...
    switch (event.key) {
      ...
      case "ArrowLeft":
        ...
        if (col === MIN_SELECTABLE_COL) { 
          ...
          newActiveCol = SELECTABLE_COL_INDICES[Math.max(0, currentIndex - 1)]; 
        }
        break;
      case "ArrowRight":
        event.preventDefault();
        if (col === MAX_SELECTABLE_COL) { 
          ...
          const currentIndex = SELECTABLE_COL_INDICES.indexOf(col);
          newActiveCol = SELECTABLE_COL_INDICES[Math.min(SELECTABLE_COL_INDICES.length - 1, currentIndex + 1)]; 
        }
        break;
      case "Tab":
        event.preventDefault();
  ...
  }, [activeCell, issues.length, selectionAnchor, updateSelectedCellsRange, internalClipboard]);
  ...
  const handleAddNewIssue = () => {
    ...
    toast({ title: "New issue added", description: "An empty issue row has been created." });
    ...
    setTimeout(() => {
        tableContainerRef.current?.focus();
        const rowElement = tableContainerRef.current?.querySelector(`[data-row-index="${newRowIndex}"]`);
  ...
  const handleImagesChange = async (id: string, files: FileList | null) => {
    ...
    try {
      ...
      toast({ title: "Images uploaded", description: `${newImages.length} image(s) added to the issue.` });
    } catch (error) {
      console.error("Error converting files to data URLs:", error);
      toast({ title: "Image Upload Error", description: "Could not process image files.", variant: "destructive" });
    }
  };

  const addSingleImageToIssue = useCallback(async (issueId: string, imageFile: File) => {
    try {
      ...
      const newImage: IssueImage = {
        ...
        url,
        file: imageFile, 
        name: imageFile.name && imageFile.name !== 'image.png' && imageFile.name !== ''
              ? imageFile.name
      ...
      toast({ title: "Image pasted", description: `Image "${newImage.name}" added to the issue.` });
    } catch (error) {
      console.error("Error converting pasted image to data URL:", error);
      toast({ title: "Image Paste Error", description: "Could not process pasted image.", variant: "destructive" });
    }
  }, [setIssues, toast]);
  ...
  const handleCopy = (id: string) => {
    setIssues((prevIssues) => {
      ...
      const newIssue: Issue = {
        ...
        images: issueToCopy.images.map(img => ({
          ...img,
          id: crypto.randomUUID(),
          file: new File([], img.name, {type: img.url.match(/data:(image\/\w+);/)?.[1] || "image/png"}),
        }))
      };
    ...
    toast({ title: "Issue copied", description: "The issue has been duplicated." });
  };
  ...
  const handleRowDrop = (targetRowId: string) => {
    ...
    const draggedItemIndex = reorderedIssues.findIndex(issue => issue.id === draggedRowId);
    let targetItemIndex = reorderedIssues.findIndex(issue => issue.id === targetRowId);
    ...
    const finalTargetIndex = reorderedIssues.findIndex(issue => issue.id === targetRowId);
    reorderedIssues.splice(finalTargetIndex, 0, draggedItem);

    setIssues(reorderedIssues);
    toast({ title: "Issue Reordered", description: "Row order has been updated." });
    setDraggedRowId(null);
    setRowDragOverId(null);
  };
  ...
  const handleSaveEditedImage = (issueId: string, imageId: string, newImageDataUrl: string) => {
    setIssues(prevIssues => 
      prevIssues.map(issue => 
        issue.id === issueId 
        ? {
            ...
            images: issue.images.map(img => 
              img.id === imageId 
              ? { ...img, url: newImageDataUrl, file: new File([], img.name, { type: newImageDataUrl.match(/data:(image\/\w+);/)?.[1] || "image/png" }) } // Update URL and recreate File
              : img
            )
          }
        : issue
    ...
    toast({ title: "Image updated", description: "Your changes have been saved." });
  ...
  if (!isClient) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-xl text-muted-foreground">Loading Issue Tracker...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <header className="mb-8">
        <h1 className="text-4xl font-headline font-bold text-primary">Issue Tracker</h1>
        <p className="text-muted-foreground">Manage your project issues with ease.</p>
      </header>
      ...
      {isImageEditorOpen && editingImageInfo && (
        <ImageEditorModal
          ...
          onSave={(newImageDataUrl) => 
            handleSaveEditedImage(editingImageInfo.issueId, editingImageInfo.image.id, newImageDataUrl)
          }
        />
      )}
    </div>
