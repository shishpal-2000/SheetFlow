...
export interface DrawingAction extends BaseAction {
    target: 'drawing';
    type: 'DRAW_PENCIL' | 'DRAW_ERASER' | 'DRAW_LINE' | 'DRAW_CURVE' | 'DRAW_CURVE_ARROW';
    payload: {
        points: Point[];
        color: string;
        strokeWidth: number;
...
export interface KonvaAction extends BaseAction {
    target: 'konva';
    type: 'ADD_RECTANGLE' | 'ADD_CIRCLE' | 'ADD_ARROW' | 'ADD_DOUBLE_ARROW' | 'ADD_TEXT' | 'MOVE_ELEMENT' | 'DELETE_ELEMENT' | 'TRANSFORM_ELEMENT' | 'MODIFY_ELEMENT';
    payload: {
